...
def show_results(img,results, img_width, img_height, model_age, model_gender, model_emotion):
	...
	cv2.imwrite('images/test.png', img_cp)

def face_detect(model, img_path):
	...
	out2 = model.predict(img_camera)[0]
...
def main(argv):
	...
	'''
	年齢・性別推定モデル　show_results走らせないなら基本的にいらない
	...
	for img_path in img_path_list:
		results = face_detect(model_face, img_path)
		print(results)
		score_list.append(is_face(results))

	print("num of data", len(score_list))
	print("num of detected data", sum(score_list))
	print("acc :", sum(score_list) / len(score_list))
